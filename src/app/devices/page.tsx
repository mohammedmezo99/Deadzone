...
import { ArrowRight, Check, Copy, RotateCcw, Search, Smartphone } from "lucide-react";
...
const imageStatusFilters = ["All", "Verified Image", "Placeholder"] as const;
...
export default function DevicesPage() {
    const familyFilters = useMemo(
        () => ["All", ...Array.from(new Set(supportedDevices.map((device) => device.family))).sort()] as const,
        [],
    );
    ...
    const [activeImageStatus, setActiveImageStatus] = useState<(typeof imageStatusFilters)[number]>("All");
    const [activeStyle, setActiveStyle] = useState<(typeof styleFilters)[number]>("All");
    ...
    const stats = useMemo(() => ({
        ...
        xiaomiDevices: supportedDevices.filter((device) => device.category === "Xiaomi" || device.category === "MIX" || device.category === "Civi").length,
        redmiDevices: supportedDevices.filter((device) => device.category === "Redmi").length,
        pocoDevices: supportedDevices.filter((device) => device.category === "POCO").length,
        padDevices: supportedDevices.filter((device) => device.category === "Pad").length,
        verifiedImages: supportedDevices.filter((device) => Boolean(device.image)).length,
        placeholderImages: supportedDevices.filter((device) => !device.image).length,
    }), []);
    ...
    const filteredDevices = useMemo(() => {
        ...
        return supportedDevices.filter((device) => {
            const imageStatus = device.image ? "Verified Image" : "Placeholder";
            const matchesSearch =
                !needle ||
            ...
            const matchesFamily = activeFamily === "All" || device.family === activeFamily;
            const matchesChipset = activeChipset === "All" || device.chipset === activeChipset;
            const matchesImageStatus = activeImageStatus === "All" || imageStatus === activeImageStatus;
            const matchesStyle = activeStyle === "All" || device.supportedStyles.includes(activeStyle);

            return matchesSearch && matchesCategory && matchesFamily && matchesChipset && matchesImageStatus && matchesStyle;
        });
    }, [activeCategory, activeChipset, activeFamily, activeImageStatus, activeStyle, query]);
            ...
            <section className="px-6 pb-20 pt-36">
                <div className="mx-auto max-w-7xl">
                    <SectionHeader
                        ...
                        description="Browse all supported Xiaomi, Redmi, POCO, MIX, Civi, and Pad devices. Search by device name, alias, or codename."
                        align="center"
                    />
                    ...
                    <div className="mb-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-7">
                        ...
                        <GlassCard accent="blue" className="p-6">
                            <p className="text-4xl font-black text-white">{stats.xiaomiDevices}</p>
                            <p className="mt-2 text-[11px] font-black uppercase tracking-[0.24em] text-blue-200">Xiaomi Devices</p>
                        </GlassCard>
                        <GlassCard accent="magenta" className="p-6">
                            <p className="text-4xl font-black text-white">{stats.redmiDevices}</p>
                            <p className="mt-2 text-[11px] font-black uppercase tracking-[0.24em] text-fuchsia-200">Redmi Devices</p>
                        </GlassCard>
                        <GlassCard accent="gold" className="p-6">
                            <p className="text-4xl font-black text-white">{stats.pocoDevices}</p>
                            <p className="mt-2 text-[11px] font-black uppercase tracking-[0.24em] text-amber-200">POCO Devices</p>
                        </GlassCard>
                        <GlassCard accent="slate" className="p-6">
                            <p className="text-4xl font-black text-white">{stats.padDevices}</p>
                            <p className="mt-2 text-[11px] font-black uppercase tracking-[0.24em] text-zinc-200">Pad Devices</p>
                        </GlassCard>
                        <GlassCard accent="cyan" className="p-6">
                            <p className="text-4xl font-black text-white">{stats.verifiedImages}</p>
                            <p className="mt-2 text-[11px] font-black uppercase tracking-[0.24em] text-cyan-200">Verified Images</p>
                        </GlassCard>
                        <GlassCard accent="red" className="p-6">
                            <p className="text-4xl font-black text-white">{stats.placeholderImages}</p>
                            <p className="mt-2 text-[11px] font-black uppercase tracking-[0.24em] text-red-200">Placeholder Images</p>
                        </GlassCard>
                    </div>
                    ...
                    <div className="mb-8 grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
                        <GlassCard accent="cyan" className="relative overflow-hidden p-6 md:p-8">
                            ...
                            <div className="relative z-10">
                                <RomBadge accent="cyan">DeadZone Device Browser</RomBadge>
                                <h2 className="mt-5 text-3xl font-black tracking-tight text-white md:text-4xl">
                                    Search the full supported-device lineup, confirm image coverage, then move directly into the DeadZone device detail page.
                                </h2>
                                <p className="mt-4 max-w-3xl text-sm leading-7 text-zinc-300 md:text-base">
                                    Filter by category, family, chipset, image status, and supported style without leaving the supported devices browser.
                                </p>
                            </div>
                        </GlassCard>

                        <GlassCard accent="magenta" className="p-6 md:p-8">
                            <p className="text-[11px] font-black uppercase tracking-[0.24em] text-fuchsia-200">Codename Help</p>
                            <h2 className="mt-3 text-2xl font-black text-white">Not sure about your codename?</h2>
                            <p className="mt-4 text-sm leading-7 text-zinc-300">
                                Always confirm your device codename before requesting or flashing any DeadZone build.
                            </p>
                    ...
                    <GlassCard accent="blue" className="sticky top-24 z-20 mb-10 p-4 md:p-5 shadow-[0_22px_60px_rgba(2,5,10,0.34)]">
                        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_1fr]">
                            ...
                            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5 xl:col-span-2">
                                <select
                                ...
                                <select
                                    value={activeImageStatus}
                                    onChange={(event) => {
                                        setActiveImageStatus(event.target.value as (typeof imageStatusFilters)[number]);
                                        setVisibleCount(visibleStep);
                                    }}
                                    className="min-h-14 rounded-2xl border border-white/10 bg-white/[0.05] px-4 text-sm font-bold text-white outline-none"
                                >
                                    {imageStatusFilters.map((filter) => <option key={filter} value={filter} className="bg-[#0a1018]">{filter === "All" ? "Image: All" : filter}</option>)}
                                </select>
                                <select
                                    value={activeStyle}
                                    onChange={(event) => {
                                ...
                                {visibleDevices.map((device, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="relative z-10 p-4 md:p-5" data-device-card="true">
                                                <DeviceImage
                                                ...
                                                <div className="mt-5 flex items-start justify-between gap-4">
                                                    <div>
                                                        <h3 className="text-xl font-black leading-tight text-white">{device.name}</h3>
                                                ...
                                                <div className="mt-4 flex flex-wrap gap-2">
                                                    ...
                                                    <RomBadge accent={device.image ? "cyan" : "red"}>{imageStatus}</RomBadge>
                                                </div>

                                                <div className="mt-5 rounded-[1.35rem] border border-white/10 bg-white/[0.04] p-3.5">
